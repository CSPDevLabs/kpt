...
echo "--> KPT: Portal in-iframe embedding (kpt-owned, no NetOpsKube overlay)"
menu="$ROOT/nok-base/portal/portal-menu-config.yaml"
if grep -q '"openInNewTab": true' "$menu" 2>/dev/null; then
  bad "nok-base portal menu still opens items in a new tab"
else
